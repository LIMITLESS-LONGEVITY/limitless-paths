...
- **Manual:** Admins can assign tiers directly on a user's record.
- **Automatic:** When a user subscribes via Stripe checkout, the webhook updates their tier automatically based on the price ID.
- **Downgrade:** When a subscription is canceled or expires, the webhook reverts the user to the Free tier.
